...
def generate_points(generator, number_of_points):
    generated_points = []
    for i in range(number_of_points):
        x = generator.integers(low=0, high=101)
        y = generator.integers(low=0, high=101)
        pickup = generator.integers(low=0, high=3) == 2
        how_much = generator.integers(100, 201)
        while point_occupied(generated_points, x, y):
            x = generator.integers(0, 101)
            y = generator.integers(0, 101)
        new_point = Point()
        new_point.id = i + 1
        new_point.x = x
        new_point.y = y
...
def update_points(generated_points, found_pickup_points, found_dropdown_points, dropdown_points):
...
def generate_warehouses(generator, number_of_warehouses, generated_points):
...
def get_total_route_length(generator, start_warehouse_point, generated_route):
    sub_routes = []
    ...
    for sub_route in sub_routes:
        ...
        if sub_routes_crosses[sub_routes.index(sub_route)] == np.min(sub_routes_crosses) \
                and generator.integers(0, 100) <= 4:
...
def plot_route(route, warehouses, save_to_png=False):
    ...
    for point in route:
        temp_color = "green"
        if route.index(point) == len(route) - 1:
            temp_color = "red"
...
def generate_routes(generator, points, warehouses, routes=1, write_generated_route_txt=False, write_route_csv=False):
    pickup_points = get_pickup_points(points)
    dropdown_points = get_dropdown_points(points)
    generated_routes = []

    for gen_route_index in range(routes):
        start_warehouse = warehouses[generator.integers(5)]
        ...
        while is_any_point_undone(points):
            ...
            update_points(points, pickup_points, dropdown_points, dropdown_points)
        temp_nearest_warehouse = find_nearest_warehouse(warehouses, actual_x, actual_y)
        temp_nearest_warehouse_point = Point()
        temp_nearest_warehouse_point.id = temp_nearest_warehouse.id
        temp_nearest_warehouse_point.x = temp_nearest_warehouse.x
        ...
        total_route_length = get_total_route_length(generator, start_warehouse, route)
        if write_generated_route_txt:
            with open(f"generated_route_{gen_route_index}.txt", "w") as file:
        ...
        if write_route_csv:
            with open(f"route_{gen_route_index}.csv", "w") as file:
                file.write("x,y\n")
                for point in route:
